...
// --- STAPPEN ---
static const GRect DIGIT_RECTS[6] = {
  {{84,  150}, {20, 30}},  // slot 0 (meest links)
  {{98,  150}, {20, 30}},  // slot 1
  {{113,  150}, {20, 30}},  // slot 2
  {{128,  150}, {20, 30}},  // slot 3
  {{143,  150}, {20, 30}},  // slot 4
  {{157, 150}, {20, 30}}   // slot 5 (meest rechts)
};

static TextLayer *s_digit_layers[6];
static char s_digit_buffers[6][2];
...
static void update_digit_layers(const char *text) {
  int len = strlen(text);

  for (int i = 0; i < 6; i++) {
    int text_index = len - 1 - i;

    if (text_index >= 0) {
      // Vul het slot met het bijbehorende karakter uit de tekst
      s_digit_buffers[5 - i][0] = text[text_index];
      s_digit_buffers[5 - i][1] = '\0';
    } else {
      // Als de tekst korter is dan 6 karakters, maken we de linkse slots leeg
      s_digit_buffers[5 - i][0] = '\0';
    }
    
    // Update de specifieke TextLayer van dit slot
    text_layer_set_text(s_digit_layers[5 - i], s_digit_buffers[5 - i]);
  }
}
...
static void update_steps() {
  ...
  // Controleer of de stappenteller beschikbaar is op het horloge
  HealthServiceAccessibilityMask mask = health_service_metric_accessible(metric, start, end);

  if (mask & HealthServiceAccessibilityMaskAvailable) {
    // Haal het échte aantal stappen van vandaag op
    HealthValue steps = health_service_sum_today(metric);
    
    snprintf(s_steps_buffer, sizeof(s_steps_buffer), "%d", (int)steps);
  } else {
    snprintf(s_steps_buffer, sizeof(s_steps_buffer), "---p");
  }

  update_digit_layers(s_steps_buffer);
}
...
static void main_window_load(Window *window) {
  ...
  // 8. Stappen
  s_steps_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_STEPS_28));

  for (int i = 0; i < 6; i++) {
    s_digit_layers[i] = text_layer_create(DIGIT_RECTS[i]);
    text_layer_set_background_color(s_digit_layers[i], GColorClear);
    text_layer_set_text_color(s_digit_layers[i], GColorFromHEX(0x7e050a));
    text_layer_set_font(s_digit_layers[i], s_steps_font);
    text_layer_set_text_alignment(s_digit_layers[i], GTextAlignmentLeft);
    text_layer_set_text(s_digit_layers[i], "");
    layer_add_child(window_layer, text_layer_get_layer(s_digit_layers[i]));
  }

  // Direct de huidige stappentelling laden
  update_steps();
}
...
static void main_window_unload(Window *window) {
  ...
  for (int i = 0; i < 6; i++) {
    text_layer_destroy(s_digit_layers[i]);
  }
